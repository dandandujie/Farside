...
本地发布构建可先通过 Kimi 官方安装器准备单文件 runtime，或用 `FARSIDE_KIMI_BINARY` 指定它；CI 则下载仓库中固定版本的 runtime，并校验固定 SHA-256 后再打包，不执行远程安装脚本。`npm run dist` 会记录版本与 SHA-256 并随安装包分发。终端用户安装 Farside 后无需另装 Kimi Code。Farside 优先启动随包 `kimi server`，仅在开发资源缺失时回退系统 PATH。
