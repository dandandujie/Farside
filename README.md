...
- **可审计 Agent Runtime**：安装包内置经版本、来源与 SHA-256 锁定的 Kimi Code 单文件 runtime；官方跟随与 Farside 自定义通道相互独立。
...
npm run runtime:check       # 检查官方 Runtime 更新
npm run dist                # 为当前操作系统生成安装包
```

本地开发可通过 Kimi 官方安装器准备同版本单文件 runtime，或用 `FARSIDE_KIMI_BINARY` 指定它；正式打包应设置 `FARSIDE_DOWNLOAD_KIMI_RUNTIME=1`。CI 会严格按 `runtime.lock.json` 下载对应通道的固定产物，并校验 SHA-256 后再打包，不执行远程安装脚本。随包 runtime 启动前还会复核内嵌锁文件、manifest 与文件完整性，服务就绪后核对 `/api/v1/meta` 的版本和能力。终端用户安装 Farside 后无需另装 Kimi Code。官方升级、自定义 Fork 和发布门禁见 [Runtime 维护指南](RUNTIME.md)。
